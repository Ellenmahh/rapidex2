...
/**
 * Created by sn1041520 on 27/09/2016.
 */
public class Lancamentos {
    ...
    public static List<Lancamentos> obterTodos(Context context) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM lancamentos l inner join categoria c " +
                " on c._id = l.idCategoria order by l.data desc ;", null);

        cursor.moveToFirst();


        for(int i=0 ; i < cursor.getCount(); i++){
            Log.d("lancamentos",cursor.getString(5) + " id " + cursor.getInt(0));
